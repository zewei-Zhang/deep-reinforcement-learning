"""
Including a convolution network and pytorch transform.
"""
import torch
import numpy as np
...
def generate_conv_layer(input_layer_par, pool_size=None, ave_pool=True):
    """
...
class CnnExtractor(nn.Module):
    def __init__(self, img_size: tuple):
        ...
        self.flatten_size = self.cal_size((1, *img_size))
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.to(self.device)

    def cal_size(self, img_size):
        """
...
def generate_transform():
    """
    Generate a torchvision compose transform, which transform the input to a grayscale and normalized tensor.
    """
    return transforms.Compose([transforms.ToTensor(), transforms.Grayscale(), transforms.Normalize(0.5, 0.5)])
